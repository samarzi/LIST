...
import {
  ...
  CheckCircle2, XCircle, FileText, Vote, RefreshCw, ExternalLink,
  Users, MessageSquare, Info, Send, Trash2, Star, Calendar,
} from 'lucide-react';
import {
  pairsApi, goalsApi, votingApi, pairMessagesApi, reportsApi, goalCommentsApi,
  type WatcherStudent, type PairStatus, type PairMessage, type GoalComment,
} from '../api/client';
...
const STATUS_LABELS: Record<string, string> = {
  ...
  on_review:   'На оценке сложности',
  on_check:    'Доказательство на проверке',
  on_voting:   'На голосовании',
  completed:   'Выполнена',
  failed:      'Провалена',
};
...
export default function WatchPage() {
  ...
  const [activeTab, setActiveTabState] = useState<'naparniks' | 'watcher'>('naparniks');
  const [pairStatus, setPairStatus] = useState<PairStatus | null>(null);
  const [students, setStudents] = useState<WatcherStudent[]>([]);
  const [loading, setLoading] = useState(true);
  const [selectedGoal, setSelectedGoal] = useState<GoalWithProof | null>(null);
  ...
  const tabs = [
    { id: 'naparniks' as const, label: 'Напарники', icon: Users },
  ...
  useEffect(() => {
    if (activeTab === 'watcher' && pairStatus?.asPartner) loadMessages();
  ...
  async function handleJoinQueue() {
    ...
    try {
      ...
      setPairStatus(pair.data);
      if (data.matched) {
        const studs = await pairsApi.myPartners();
        setStudents(studs.data);
      }
    } catch (err) {
  ...
  const reviewGoals = students.flatMap(ws => ws.partner.activeGoals.filter(g => g.status === 'on_review'));
  const proofGoals  = students.flatMap(ws => ws.partner.activeGoals.filter(g => g.status === 'on_check'));
    ...
    <div className="page-content">
      ...
      <div style={{ padding: '24px 16px 12px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div>
          ...
          <p className="body-sm text-faint mt-2">Напарники и смотрящий</p>
        </div>
        <motion.button
          whileTap={{ scale: 0.9 }}
          ...
          <>
              ...
              {proofGoals.length > 0 && (
                ...
                >
                  <div style={{
                    ...
                    border: '1px solid rgba(48,209,88,0.2)', borderRadius: 14,
                    ...
                    <div>
                      ...
                      <div style={{ fontSize: 12, color: 'rgba(48,209,88,0.6)' }}>
            ...
            <AnimatePresence>
              {reviewGoals.length > 0 && (
                ...
                >
                  <div style={{
                    padding: '12px 14px', background: 'rgba(255,214,10,0.08)',
                    ...
                    <div>
                      ...
                      <div style={{ fontSize: 12, color: 'rgba(255,214,10,0.7)' }}>
            ...
            {students.length === 0 ? (
              <div style={{ padding: '0 16px' }}>
                <div className="empty-state">
                  ...
                  <div>
                    <h3 className="title-sm">Нет напарников</h3>
                    <p className="body-sm text-faint mt-1">Найди напарника чтобы следить за его прогрессом</p>
                    <button
                    ...
                    >
                      {joiningQueue ? 'Поиск...' : 'Найти напарника'}
                    </button>
                  </div>
                </div>
              </div>
          ...
          <>
            ...
            {pairStatus?.asPartner ? (
              <div style={{ padding: '0 16px' }}>
                ...
                {/* Messages */}
                <div className="label text-faint mb-3">Доработки и вопросы от смотрящего</div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 14 }}>
                  {messages.length === 0 ? (
                    <div className="empty-state" style={{ padding: '20px 0' }}>
                      <div className="empty-icon"><MessageSquare size={24} color="var(--text-3)" /></div>
                      <p className="body-sm text-faint">Здесь появятся сообщения от смотрящего</p>
                    </div>
                  ) : (
                    messages.map(msg => (
                      <motion.div
                        ...
                        style={{
                          padding: '10px 13px',
                          background: msg.isFromMe ? 'var(--surface-2)' : 'rgba(167,139,250,0.07)',
                      ...
                      >
                        ...
                        <div style={{ fontSize: 14, lineHeight: 1.4, color: 'var(--text-1)' }}>{msg.content}</div>
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  <input
                    ...
                    placeholder="Написать смотрящему..."
                    value={newMessage}
                  ...
                  <button
                    ...
                    style={{
                      ...
                      opacity: sendingMessage ? 0.6 : 1, flexShrink: 0,
                    }}
                  >
      ...
      <AnimatePresence>
        {selectedGoal && (
          <GoalDetailModal
            ...
            onDifficultySet={(d) => handleDifficultySet(selectedGoal.id, d)}
            onProofDecide={(confirmed, note) => handleProofDecision(selectedGoal.id, confirmed, note)}
          />
        )}
      </AnimatePresence>
      ...
      <AnimatePresence>
        {showChangePair && (
          <ChangePairModal onClose={() => setShowChangePair(false)} onConfirm={handleChangePair} />
        )}
      </AnimatePresence>
            ...
            >
              ...
              <div style={{ fontSize: 14, lineHeight: 1.7, color: 'var(--text-2)' }}>
                <p style={{ marginBottom: 14 }}>
                  <strong style={{ color: 'var(--text-1)' }}>Напарники:</strong> люди, чьи цели ты проверяешь как смотрящий. Оцени сложность и подтверждай доказательства.
                </p>
                <p style={{ marginBottom: 14 }}>
                  <strong style={{ color: 'var(--text-1)' }}>Смотрящий:</strong> твой смотрящий проверяет твои цели и помогает тебе расти. Общайся с ним через сообщения.
                </p>
                <p style={{ color: 'var(--text-3)', fontSize: 13 }}>
                  Система матчинга автоматически подбирает напарников на основе твоего уровня.
                </p>
              </div>
            </motion.div>
          </motion.div>
...
function NaparnikCard({
  ws, index, onGoalClick,
}: {
  ws: WatcherStudent;
  index: number;
  onGoalClick: (goal: GoalWithProof) => void;
}) {
  const { partner } = ws;
  const name = partner.displayName ?? partner.username ?? 'Напарник';
  ...
  return (
    ...
    >
      ...
      {partner.activeGoals.length === 0 ? (
        <div className="body-sm text-faint text-center" style={{ padding: '6px 0 2px' }}>
          Нет активных целей
        </div>
      ) : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {partner.activeGoals.map(goal => {
            ...
            const urgent = needsReview || needsProof;
            ...
            return (
              <motion.div
                ...
                style={{
                  ...
                  background: needsProof
                    ...
                    : 'var(--surface-2)',
                  border: `1px solid ${
                    needsProof ? 'rgba(48,209,88,0.2)' :
                    needsReview ? 'rgba(255,214,10,0.2)' :
                    'transparent'
                  }`,
                  borderRadius: 12,
                  cursor: 'pointer',
              ...
              >
                <div className="flex items-start justify-between gap-2">
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div className="body-sm" style={{ fontWeight: 600, marginBottom: 6, lineHeight: 1.3 }}>
                      {goal.title}
                    </div>
                    <div className="flex items-center gap-3">
                      <div className="flex items-center gap-1" style={{ color: daysLeft <= 3 ? 'var(--yellow)' : 'var(--text-3)' }}>
                        <Clock size={11} />
                        <span style={{ fontSize: 11 }}>{daysLeft > 0 ? `${daysLeft} дн` : 'Просрочена'}</span>
                      </div>
                      {goal.checkinsCount > 0 && (
                        <div className="flex items-center gap-1 text-faint">
                          <TrendingUp size={11} />
                          <span style={{ fontSize: 11 }}>{goal.checkinsCount} чекинов</span>
                        </div>
                      )}
                    </div>
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexShrink: 0 }}>
                    {urgent ? (
                      <div style={{
                        ...
                        background: needsProof ? 'rgba(48,209,88,0.15)' : 'rgba(255,214,10,0.15)',
                        fontSize: 11, fontWeight: 700,
                        color: needsProof ? 'var(--green)' : 'var(--yellow)',
                      }}>
                        {needsProof ? 'Проверить' : 'Оценить'}
                      </div>
                    ) : (
...
function GoalDetailModal({
  ...
  goal: GoalWithProof;
  onClose: () => void;
  onDifficultySet: (d: number) => Promise<void>;
  onProofDecide: (confirmed: boolean, note?: string) => Promise<void>;
}) {
  const [view, setView] = useState<'detail' | 'difficulty' | 'proof' | 'comments'>('detail');
  ...
  async function loadComments() {
    ...
    try {
      const { data } = await goalCommentsApi.list(goal.id);
      setComments(data);
    } finally {
  ...
  async function sendComment() {
    ...
    try {
      const { data } = await goalCommentsApi.create(goal.id, commentText.trim());
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div className="flex items-center gap-2 mb-1">
              {view !== 'detail' && (
                ...
                >
                  <ChevronRight size={16} color="var(--text-3)" style={{ transform: 'rotate(180deg)' }} />
                </button>
              )}
              <span style={{ fontSize: 12, color: 'var(--text-3)', fontWeight: 500 }}>
                {view === 'detail' ? STATUS_LABELS[goal.status] ?? goal.status :
                 view === 'difficulty' ? 'Оценка сложности' :
                 view === 'proof' ? 'Проверка доказательства' : 'Комментарии'}
              </span>
            </div>
            <h2 className="title-sm truncate">{goal.title}</h2>
          </div>
          <button className="btn-icon btn-ghost" onClick={onClose} style={{ marginLeft: 8, flexShrink: 0 }}>
            <X size={18} />
          </button>
        </div>
          ...
          <AnimatePresence mode="wait">
              ...
              >
                ...
                <div style={{ display: 'flex', gap: 8, marginBottom: 16, flexWrap: 'wrap' }}>
                  <div style={{
                    ...
                    fontSize: 12, color: daysLeft <= 3 ? 'var(--yellow)' : 'var(--text-2)',
                  }}>
                    <Calendar size={12} />
                    {daysLeft > 0 ? `${daysLeft} дней осталось` : 'Дедлайн прошёл'}
                  </div>
                  {goal.checkinsCount > 0 && (
                    <div style={{
                      display: 'flex', alignItems: 'center', gap: 5, padding: '5px 10px',
                      background: 'var(--surface-2)', borderRadius: 8, fontSize: 12, color: 'var(--text-2)',
                    }}>
                      <Star size={12} />
                      {goal.checkinsCount} чекинов
                    </div>
                  )}
                </div>
                ...
                <div style={{
                  ...
                  <p className="body-sm" style={{ lineHeight: 1.6, color: 'var(--text-2)' }}>
                ...
                {goal.latestProof && (
                  <div style={{
                    padding: '12px 14px', background: 'rgba(48,209,88,0.06)',
                    border: '1px solid rgba(48,209,88,0.15)', borderRadius: 12, marginBottom: 12,
                  }}>
                    <div className="label text-faint mb-2">Доказательство</div>
                    <p className="body-sm" style={{ lineHeight: 1.5, color: 'var(--text-2)' }}>
                      {goal.latestProof.description}
                    </p>
                    {goal.latestProof.mediaUrls?.length > 0 && (
                      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginTop: 10 }}>
                        {goal.latestProof.mediaUrls.map((url, i) => (
                          <a key={i} href={url} target="_blank" rel="noopener noreferrer"
                            style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 12, color: 'var(--accent)', fontWeight: 500 }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginTop: 4 }}>
                  ...
                  <button
                    ...
                    onClick={() => { setView('comments'); loadComments(); }}
                  >
            ...
            {view === 'difficulty' && (
              <motion.div
                ...
                initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}
              >
                <DifficultyView goal={goal} onSave={onDifficultySet} />
              </motion.div>
            )}
            ...
            {view === 'proof' && (
              <motion.div
                ...
                initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}
              >
                <ProofView goal={goal} onDecide={onProofDecide} />
              </motion.div>
            )}
            ...
            {view === 'comments' && (
              ...
              >
                {commentsLoading ? (
                  ...
                  <div className="body-sm text-faint text-center" style={{ paddingTop: 20 }}>Нет комментариев</div>
                ) : (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 14 }}>
                    {comments.map(c => (
                      <div key={c.id} style={{
                        ...
                        <div style={{ fontSize: 11, color: 'var(--text-3)', marginBottom: 3 }}>
                          {c.isFromMe ? 'Вы' : (c.user.displayName || c.user.username || 'Напарник')}
                        </div>
                ...
                <div style={{ display: 'flex', gap: 8, marginTop: 'auto', paddingTop: 8 }}>
                  ...
                  <button
                    ...
                    style={{
                      width: 40, height: 40, borderRadius: 10,
                      background: commentText.trim() ? 'var(--accent)' : 'var(--surface-2)',
                      border: 'none', cursor: commentText.trim() ? 'pointer' : 'not-allowed',
                      display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0,
                    }}
                  >
                    <Send size={18} color={commentText.trim() ? '#fff' : 'var(--text-3)'} />
                  </button>
                </div>
              </motion.div>
            )}
...
function DifficultyView({ goal, onSave }: { goal: GoalWithProof; onSave: (d: number) => Promise<void> }) {
  ...
  async function handleSave() {
    setSaving(true);
    try { await onSave(difficulty); } finally { setSaving(false); }
  }

  return (
    <div>
      <div style={{
        padding: '10px 14px', background: 'var(--surface-2)', borderRadius: 12, marginBottom: 20,
      }}>
      ...
      <div style={{ textAlign: 'center', marginBottom: 20 }}>
        <motion.div
          ...
          style={{ fontSize: 56, fontWeight: 800, color: hint.color, lineHeight: 1 }}
        >
          {difficulty}
        </motion.div>
        <motion.div
          ...
          style={{ fontSize: 16, fontWeight: 600, color: hint.color, marginTop: 6 }}
        >
          {hint.label}
        </motion.div>
      ...
      <div style={{ marginBottom: 16 }}>
        <div className="score-track" style={{ marginBottom: 8 }}>
          <div className="score-track-fill" style={{
            width: `${(difficulty / 10) * 100}%`,
            background: `linear-gradient(90deg, ${hint.color}88, ${hint.color})`,
          }} />
        </div>
        <input type="range" min={0} max={10} step={1} value={difficulty} onChange={e => setDifficulty(Number(e.target.value))} />
      ...
      <AnimatePresence>
        {difficulty === 0 && (
          <motion.div
            ...
            style={{
              padding: '10px 12px', background: 'rgba(255,69,58,0.08)', border: '1px solid rgba(255,69,58,0.2)',
              borderRadius: 10, marginBottom: 16, fontSize: 12, color: 'var(--red)', lineHeight: 1.5,
            }}
          >
            ⚠ Оценка 0 вернёт цель напарнику на доработку
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <button className="btn btn-primary btn-full" onClick={handleSave} disabled={saving}>
        <Flame size={16} />
        {saving ? 'Сохраняю...' : `Поставить сложность ${difficulty}`}
      </button>
    </div>
  );
}
...
function ProofView({
  ...
  async function decide(confirmed: boolean) {
    setLoading(true);
    try { await onDecide(confirmed, note || undefined); } finally { setLoading(false); }
  }
  ...
  return (
    <div>
      {/* Goal info */}
      <div style={{ padding: '10px 14px', background: 'var(--surface-2)', borderRadius: 12, marginBottom: 12 }}>
        <div className="label text-faint mb-1">Критерии успеха</div>
        <p className="body-sm" style={{ lineHeight: 1.5, color: 'var(--text-2)' }}>{goal.successCriteria}</p>
      </div>
      ...
      {proof ? (
        <div style={{
          padding: '12px 14px', background: 'rgba(48,209,88,0.06)',
          border: '1px solid rgba(48,209,88,0.15)', borderRadius: 12, marginBottom: 14,
        }}>
          <div className="label text-faint mb-2">Доказательство от напарника</div>
          <p className="body-sm" style={{ lineHeight: 1.5, color: 'var(--text-2)' }}>{proof.description}</p>
        ...
        <div style={{
          ...
          borderRadius: 12, marginBottom: 14, fontSize: 13, color: 'var(--red)',
        }}>
      ...
      {step === 'reject' && (
        <div style={{ marginBottom: 14 }}>
          ...
          <textarea className="input" placeholder="Что нужно доработать?" value={note} onChange={e => setNote(e.target.value)} rows={3} />
        </div>
      )}
      ...
      {step === 'report' && (
        <div style={{ marginBottom: 14 }}>
          {reportSent ? (
            <div style={{
              padding: '12px 14px', background: 'rgba(48,209,88,0.06)', border: '1px solid rgba(48,209,88,0.2)',
              borderRadius: 12, fontSize: 13, color: 'var(--green)', textAlign: 'center',
            }}>
            ...
            <>
              ...
              <textarea className="input" placeholder="Опиши, что не так..." value={reportReason} onChange={e => setReportReason(e.target.value)} rows={3} />
            </>
          )}
        </div>
      )}
      ...
      {step === 'view' ? (
        <>
          <div className="flex gap-2">
            <button className="btn btn-ghost" style={{ flex: 1, color: 'var(--red)', borderColor: 'rgba(255,69,58,0.3)' }}
              onClick={() => setStep('reject')} disabled={loading}>
              <XCircle size={16} />
              Отклонить
            </button>
            <button className="btn btn-primary" style={{ flex: 1, background: 'linear-gradient(135deg,#30d158,#34c759)' }}
              onClick={() => decide(true)} disabled={loading}>
        ...
        <div className="flex gap-2">
          <button className="btn btn-ghost" style={{ flex: 0.4 }} onClick={() => setStep('view')} disabled={loading || reportSent}>Назад</button>
          {reportSent ? (
            <button className="btn btn-primary" style={{ flex: 1 }} onClick={() => setStep('view')}>Закрыть</button>
          ) : (
            <button className="btn" onClick={submitReport} disabled={loading || reportReason.trim().length < 10}
              style={{ flex: 1, background: 'rgba(255,69,58,0.12)', color: 'var(--red)', border: '1px solid rgba(255,69,58,0.3)', borderRadius: 12, opacity: reportReason.trim().length < 10 ? 0.5 : 1 }}>
        ...
        <div className="flex gap-2">
          <button className="btn btn-ghost" style={{ flex: 0.4 }} onClick={() => setStep('view')} disabled={loading}>Назад</button>
          <button className="btn" onClick={() => decide(false)} disabled={loading}
    ...
    >
      ...
      >
        ...
        <div style={{ marginBottom: 16 }}>
          ...
          <textarea className="input" placeholder="Почему хочешь сменить смотрящего?" value={reason} onChange={e => setReason(e.target.value)} rows={3} />
        </div>
...
function UserAvatar({ user, size }: {
  ...
  return (
    <div className="avatar-placeholder" style={{ width: size, height: size, fontSize: size * 0.35, flexShrink: 0 }}>
...
function FrozenPairBanner({ onReturn }: { onReturn: () => Promise<void> }) {
  ...
  async function handleReturn() {
    setLoading(true);
    try { await onReturn(); } finally { setLoading(false); }
  }
  return (
    <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} style={{ margin: '0 16px 16px' }}>
      <div style={{
        padding: '14px 16px', background: 'rgba(90,120,200,0.08)',
        border: '1px solid rgba(90,120,200,0.25)', borderRadius: 14,
      }}>
        <div style={{ fontSize: 14, fontWeight: 600, color: '#7b9cff', marginBottom: 4 }}>Пара заморожена</div>
        ...
        <button onClick={handleReturn} disabled={loading} style={{
          padding: '8px 16px', background: '#7b9cff', color: '#fff', border: 'none',
